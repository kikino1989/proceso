...
export class DatabaseService {
    ...
    runMigrations(db) {
        return db.transaction(async tx => {
            await this.insertDefaults(tx, "Book", Book.getDefaultBooks());
            await this.insertDefaults(tx, "Habit", Habit.getDefaultHabits());
            await this.insertDefaults(tx, "ProspectingSteps", ProspectingSteps.getDefaultSteps());
        });
    }

    insertDefaults(tx, tableName, defaults) {
